class Cliente < ActiveRecord::Base
  ...
  validates :num_identidad, :length => { :minimum => 6, :maximum => 8}, :uniqueness => true, :format => { :with =>  /\d/}
  ...
  validates :telefono, :length => { :minimum => 6, :maximum => 13}, :format => { :with =>  /^[0-9]{2,3}-? ?[0-9]{6,7}$/ }
